...
apply_tidy_msg <- function(x){
  ...
  # set block info in
  t3 <- map(block_info$keywords, which_block, msglist=raw_msg)
  ...
  minl <- unlist(map(raw_msg$block_name, trial_j, b=block_info))
  ...
  time_info$stim1_end_p <- c("b1a_q1$", "b1b_mask$", "b2_star$", NA, "b4a_keyboard$", "b4b_keyboard$", "b5a_keyboard$", "^0 scale_start$")
  ...
  time_info$stim1_2_start_p <- c(NA, "b1b_blank$", NA, NA, NA, NA, NA, NA)
  time_info$stim1_2_end_p <- c(NA, "b1b_q1$", NA, NA, NA, NA, NA, NA)
  ...
  stim_1_2_start <- get_time2(time_info$stim1_2_start_p, "stim_1_2_start")
  # get end time
  ...
  stim_1_2_end <- get_time2(time_info$stim1_2_end_p, "stim_1_2_end")
  # merge all time info
  time_list <- list(stim_1_start, stim_2_start, stim_3_start, stim_4_start, stim_5_start, stim_1_2_start, stim_1_end, stim_2_end, stim_3_end, stim_4_end, stim_5_end, stim_1_2_end)
  
  # output tidy msg
  all_msg <- time_list %>% reduce(full_join) %>%
    full_join(all_msg) %>%
    arrange(block) %>%
    select(block, block_name, trial_id, trial_name, type, k1, k2, k3, k4, acc, rt, stim_1_start, stim_1_end, stim_2_start, stim_2_end, stim_3_start, stim_3_end, stim_4_start, stim_4_end, stim_5_start, stim_5_end, stim_1_2_start, stim_1_2_end)
  all_msg$participant_group <- x$participant_group$group
  all_msg$participant_phone <- x$participant_phone$phone
  b3_fix <- all_msg %>% filter(block_name=="b3") %>%
