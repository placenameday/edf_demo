...
# 读取左右区域aoi信息
left_aoi <- read_csv("/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/tidy/AOI_info/b2_left.csv")
right_aoi <- read_csv("/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/tidy/AOI_info/b2_right.csv")
# 读取刺激属性的左右信息
...
for (i in unique(b2_aoi_info$left)) {
  ...
  i_r <- b2_aoi_info %>% filter(right == i) %>% select(-left, -right) %>%
    left_join(right_aoi)
  new_aoi_file <- paste("/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/tidy/AOI_info/b2_", i, ".csv", sep = "")
...
for (i in unique(star_aoi_info$left)) {
  ...
  i_r <- star_aoi_info %>% filter(right == i) %>% select(-left, -right) %>%
    left_join(right_aoi)
  new_aoi_file2 <- paste("/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/tidy/AOI_info/b2_", i, ".csv", sep = "")
...
```{r, message=FALSE, warning=FALSE}
# 读取四个区域aoi信息
lt_aoi <- read_csv("/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/tidy/AOI_info/b3_lt.csv")
rt_aoi <- read_csv("/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/tidy/AOI_info/b3_rt.csv")
lb_aoi <- read_csv("/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/tidy/AOI_info/b3_lb.csv")
rb_aoi <- read_csv("/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/tidy/AOI_info/b3_rb.csv")
...
for (i in unique(b3_aoi_info$one)) {
  ...
  i_br <- b3_aoi_info %>% filter(four == i) %>% select(-one, -two, -three, -four) %>%
    left_join(rb_aoi)
  new_aoi_file3 <- paste("/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/tidy/AOI_info/b3_", i, ".csv", sep = "")
