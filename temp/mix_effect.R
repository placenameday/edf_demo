...
# 结果输出
mixed_maas = lmer(fc ~ maas*AOI + (1 | participant_phone), data = dtb3_r4)
...
summary(mixed_two)
# maas 结果
maas_t <- tab_model(mixed_maas, dv.labels = metri)
...
# sas 结果
sas_t <- tab_model(mixed_sas, dv.labels = metri)
...
# two 结果
two_t <- tab_model(mixed_two, dv.labels = metri)
