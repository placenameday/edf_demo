...
anv_a <- function(metri, stname, bname, dt3, aoi){
  ...
  if (length(b1a$fc)>30) {
    ...
    # 方差分析结果
    aov.manu <- try(aov(fc ~ maasg + tgjlg + sasg + maasg:tgjlg:sasg + maasg:tgjlg + tgjlg:sasg + maasg:sasg, data=b1a))
    summary(aov.manu)[[1]] %>% mutate(block_name=bname, stim=stname, AOI=aoi, metric=mm, effect=rownames(.), ztx=ztjy, qx=qxjy) %>%
      rename(p = "Pr(>F)") %>% mutate(p=round(p,4), Resdf=summary(aov.manu)[[1]]["Residuals",1])
  } else NULL
}
...
alm_a <- function(metri, stname, bname, dt3, aoi){
  ...
  if (length(dtc$fc)>30) {
    ...
    sl <- as.data.frame(summary(ldtc)["coefficients"][[1]]) %>% mutate(block_name=bname, stim=stname, AOI=aoi, metric=mm, effect=rownames(.), ztx=ztjy, df=summary(ldtc)[["df"]][2], ccfb=skewness) %>%
      rename(p = "Pr(>|t|)") %>% mutate(p=round(p,4)) %>% filter(effect!="(Intercept)")
  } else NULL
}
