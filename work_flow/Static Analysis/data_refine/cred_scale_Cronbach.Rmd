...
dt2 <- read_csv("/Users/placenameday/R study/edf_demo/data/processed/sclae_cred/processed/all_4.csv") %>%
  rename(Anxiety=tzjl, MAAS=maas, SAS=phoneadd)

# 数据形态转换
dt3 <- dt2 %>% select(participant_phone, ztjl, Anxiety, MAAS, SAS) %>%
  pivot_longer(2:5, names_to = "scale", values_to = "score") %>%
  filter(scale!="ztjl") %>%
  mutate(scale=as.factor(scale))

dt4 <- dt2 %>% select(Anxiety, MAAS, SAS)
...
dt2 <- na.omit(dt2)
# maas 对 tzjl的直接效应
fit.totaleffect=lm(Anxiety~MAAS,dt2)
summary(fit.totaleffect)

# maas 对 sas的效应
fit.mediator=lm(SAS~MAAS,dt2)
summary(fit.mediator)

# maas 作为 mediator 对tzjl的效应
fit.dv=lm(Anxiety~MAAS+SAS,dt2)
summary(fit.dv)

results = mediate(fit.mediator, fit.dv, treat='MAAS', mediator='SAS', boot=T)
summary(results)
```
