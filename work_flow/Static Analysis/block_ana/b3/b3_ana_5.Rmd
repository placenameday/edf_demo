...
mix_e_f <- function(dt, bname, sname, a, metri){
  ...
  dtc2 <- dt %>% filter(block_name==bname, stim==sname, AOI %in% a) %>%
    mutate(AOI=case_when(AOI == "b3_ne"~"ne",
                         ...
                         AOI == "b3_neg"~"neg",
                         AOI == "b3_thr"~"thr")) %>%
    group_by(participant_phone, trial_name, AOI) %>%
    summarise(fc=mean(!!sym(metri), na.rm=T), maas=mean(maas), tgjl=mean(tgjl), sas=mean(sas)) %>%
    filter(!is.na(fc)) %>%
    ungroup() %>%
    convert_as_factor(participant_phone, trial_name, AOI) %>%
    select(participant_phone, trial_name, AOI, tgjl, maas, sas, fc)
  ...
  # 模型拟合
  mixed_maas = lmer(fc ~ maas+AOI+maas*AOI + (1 | participant_phone)+ (1 | trial_name), data = dtb3_r4)
  mixed_sas = lmer(fc ~ sas+AOI+sas*AOI + (1 | participant_phone)+ (1 | trial_name), data = dtb3_r4)
  mixed_two = lmer(fc ~ maas+sas+AOI+maas*AOI+sas*AOI + (1 | participant_phone)+ (1 | trial_name), data = dtb3_r4)
...
dt <- read_csv("/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/ea/data/inner_dt_aoi_re_2.csv")
```
