...
#### fangfa
```{r, message=FALSE, warning=FALSE}
fc <- function(dt, bname, sname, a, metri){
  dtb3 <- dt %>% filter(block_name %in% bname, stim==sname, AOI %in% a) %>%
    mutate(maas=maas_g, sas=sas_g) %>%
    filter(maas != "mid", sas != "mid") %>%
    group_by(participant_phone, maas, sas) %>%
    summarise(fc=mean(!!sym(metri), rm.na=T)) %>%
    select(participant_phone, maas, sas, fc) %>%
    convert_as_factor(participant_phone, maas, sas) %>% ungroup()
  ...
  ztxp <- ggqqplot(dtb3_r4, "fc", ggtheme = theme_bw()) +
    facet_grid(maas ~ sas, labeller = "label_both")
...
## 分析
```{r, message=FALSE, warning=FALSE}
map(mlist, fc, dt=dt, bname=bname, sname=sname, a=a)
```