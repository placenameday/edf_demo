...
#### fangfa
```{r, message=FALSE, warning=FALSE, out.width = "100%", fig.align = "center", out.height = "100%"}
mix_e_f <- function(dt, bname, sname, a, metri){
  ...
  maas_check <- check_model(mixed_maas, panel=F)
  ...
  maas_results <- list(table=maas_t, maas_perfor=maas_perfor, p1=maas_p1, p2=maas_p2, maas_check=maas_check)
  
  # sas 结果
  sas_t <- model_parameters(mixed_sas)
  sas_check <- check_model(mixed_sas, panel=F)
  ...
  sas_results <- list(table=sas_t, sas_perfor=sas_perfor, p1=sas_p1, p2=sas_p2, sas_check=sas_check)
  
  # two 结果
  two_t <- model_parameters(mixed_two)
  two_check <- check_model(mixed_two, panel=F)
  ...
  two_results <- list(table=two_t, two_perfor=two_perfor, p1=two_p1, p2=two_p2, p3=two_p3, two_check=two_check)
