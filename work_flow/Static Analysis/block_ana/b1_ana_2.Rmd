...
## 对b1进行 量表得分 x aoi x ab处理 混合重复测量方差分析
...
#### fangfa
```{r, message=FALSE, warning=FALSE}
b1test <- function(bname,sname,a,group, metri,dt){
...
## 景深aoi上的交互 tgjl
```{r, message=FALSE, warning=FALSE}
map(mlist, possibly(b1test, otherwise = NA_character_), dt=dt, bname=block, sname=stim, a=aoilist, group=group)
```

## 景深aoi上的交互 maas
```{r, message=FALSE, warning=FALSE}
map(mlist, possibly(b1test, otherwise = NA_character_), dt=dt, bname=block, sname=stim, a=aoilist, group="maas_g")
```

## 景深aoi上的交互 sas
```{r, message=FALSE, warning=FALSE}
map(mlist, possibly(b1test, otherwise = NA_character_), dt=dt, bname=block, sname=stim, a=aoilist, group="sas_g")
```