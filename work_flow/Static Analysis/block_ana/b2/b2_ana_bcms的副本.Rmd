...
mix_e_f <- function(dt, bname, sname, metri){
  dtc2 <- dt %>% filter(block_name==bname, stim%in%sname) %>%
  ...
  # 结果汇总
  maas_results <- list(maas_t=maas_t, maas_perfor=maas_perfor, maas_jh=maas_jh, maas_eta=maas_eta, p1=maas_p1, p2=maas_p2)
  ...
  # 结果汇总
  sas_results <- list(sas_t=sas_t, sas_perfor=sas_perfor, sas_jh=sas_jh, sas_eta=sas_eta, p1=sas_p1, p2=sas_p2)
  ...
  two_results <- list(two_t=two_t, two_perfor=two_perfor, two_jh=two_jh, two_eta=two_eta, two_inter_v_m=two_inter_v_m,two_inter_v_s=two_inter_v_s,  p1=two_p1, p2=two_p2, two_p3=two_p3)
