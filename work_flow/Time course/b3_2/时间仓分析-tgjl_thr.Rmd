...
## 摘要
针对注视比例出时序图并进行mixed模型差异分析, 按照特质焦虑进行分组，进一步优化分析,只分析thr
...
ggplot(dt2, aes(x=Time, y=Prop, color=Target)) +
...
mm2 <- function(dt, t){
  ...
  model <- lmer(Prop ~ Target + (1| participant_phone) + (1|trial_id),  data = dt4)
  
  pwc <- summary(pairs(emmeans(model, "Target", lmerTest.limit = 10000))) %>% filter(p.value <0.05) %>%
    mutate(Time=t)
  pwc
}
