...
model.1 <- lmer(Elog ~ (ot1+ot2+ot3+ot4)*AOI*Target + (1| participant_phone), data = dt3_1)
...
p.1 <- ggplot(dt3_1, aes(x=Time2/1000, y=Elog, color=AOI)) +
                     ...
                     scale_x_continuous(breaks = c(0,2,4,6,7.9))



p.2 <- ggplot(dt3_2, aes(x=Time2/1000, y=Elog, color=AOI)) +
                     facet_grid(rows = vars(Target)) +
                     stat_summary(fun.data=mean_se, geom="pointrange",alpha=0.65, size=0.1) +
                     ...
                     coord_cartesian(ylim = c(-3.5, -0.5)) +
                     scale_x_continuous(breaks = c(7.9,10,12,14,16,17.7))

p.3 <- ggplot(dt3_3, aes(x=Time2/1000, y=Elog, color=AOI)) +
                     facet_grid(rows = vars(Target)) +
                     stat_summary(fun.data=mean_se, geom="pointrange",alpha=0.65, size=0.1) +
                     ...
                     theme(axis.ticks=element_blank(),
                           ...
                           axis.title.y=element_blank()) +
                     coord_cartesian(ylim = c(-3.5, -0.5)) +
                     scale_x_continuous(breaks = c(17.7,20,22,24,26,28,30))
...
```{r, message=FALSE, warning=FALSE}
p.1.2 <- ggplot(dt3_1, aes(x=Time2/1000, y=Elog, color=Target)) +
                     facet_wrap(vars(AOI), nrow = 2) +
                     stat_summary(fun.data=mean_se, geom="pointrange",alpha=0.65, size=0.005) +
                     stat_summary(aes(y=predict(model.1,dt3_1,re.form=NA)), fun.y=mean, geom="line",size=1,alpha=0.75) +
                     labs(x="Time(s)") +
                     labs(colour = "Stimuli") +
                     theme(axis.text.x=element_text(face = "bold"),
...
p11 <- p.1.2 + ggsci::scale_color_nejm()
p11

p.2.2 <- ggplot(dt3_2, aes(x=Time2/1000, y=Elog, color=Target)) +
                     facet_wrap(vars(AOI), nrow = 2) +
                     stat_summary(fun.data=mean_se, geom="pointrange",alpha=0.65, size=0.005) +
                     stat_summary(aes(y=predict(model.2,dt3_2,re.form=NA)), fun.y=mean, geom="line",size=1,alpha=0.75) +
                     labs(x="Time(s)") +
...
p12 <- p.2.2 + ggsci::scale_color_nejm()
p12

p.3.2 <- ggplot(dt3_3, aes(x=Time2/1000, y=Elog, color=Target)) +
                     facet_wrap(vars(AOI), nrow = 2) +
                     stat_summary(fun.data=mean_se, geom="pointrange",alpha=0.65, size=0.005) +
                     stat_summary(aes(y=predict(model.3,dt3_3,re.form=NA)), fun.y=mean, geom="line",size=1,alpha=0.75) +
                     labs(x="Time(s)") +
...
p13 <- p.3.2 + ggsci::scale_color_nejm()


p222 <- ((p11 + plot_layout(guides = "collect") & theme(legend.position = "top")) + plot_spacer() + (p12 + theme(legend.position = "none")) + plot_spacer() + (p13 + theme(legend.position = "none")))
p222

fig32 <- p222 + plot_layout(widths = c(4, -0.3 ,4,-0.3 ,4), guides = "collect")
fig32
ggsave(filename = "/Users/placenameday/Downloads/newFig/fig4.pdf", width = 180, height = 120, units = "mm")
```
...
fig3 <- p123 + plot_layout(widths = c(4, -0.3 ,4.5,-0.3 ,4), guides = "collect")
fig3
# ggsave(filename = "/Users/placenameday/Downloads/fig3a.pdf", width = 180, height = 120, units = "mm")
```
