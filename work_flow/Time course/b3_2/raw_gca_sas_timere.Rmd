...
dt3_2 <- dt3 %>% filter(between(Time,7900,17300)) %>% mutate(Time2=Time, Time=Time-7900)
dt3_3 <- dt3 %>% filter(between(Time,17300,25700)) %>% mutate(Time2=Time, Time=Time-17300)
...
write_csv(dt3_1, "/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/ea/data/b3_1_ot_sas.csv")
dt3_2 <- time_code_m(dt3_2)
write_csv(dt3_2, "/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/ea/data/b3_2_ot_sas.csv")
dt3_3 <- time_code_m(dt3_3)
write_csv(dt3_3, "/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/ea/data/b3_3_ot_sas.csv")
...
make.model <- function(dt){
  model.base <- lmer(Elog ~ (ot1+ot2+ot3) + (1| participant_phone), data = dt)
  model.0 <- lmer(Elog ~ (ot1+ot2+ot3)+AOI+Target + (1| participant_phone), data = dt)
