...
dt <- dt %>% full_join(dt_0) %>% mutate(ps= ps- psz)

dt_all <- dt
dt_pos <- dt %>% filter(AOI == "threat")
...
make.model <- function(dt){
  model.base <- lmer(ps ~ (ot1+ot2+ot3) + (1| participant_phone), data = dt)
  model.0 <- lmer(ps ~ (ot1+ot2+ot3)+Target + (1| participant_phone), data = dt)
  model.1 <- lmer(ps ~ ot1*Target + (1| participant_phone), data = dt)
  model.2 <- lmer(ps ~ (ot1+ot2)*Target + (1| participant_phone), data = dt)
  model.3 <- lmer(ps ~ (ot1+ot2+ot3)*Target + (1| participant_phone), data = dt)
