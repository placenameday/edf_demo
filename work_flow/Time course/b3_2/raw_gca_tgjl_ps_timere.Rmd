...
dt3_2 <- dt3 %>% filter(between(Time,7900,18000)) %>% mutate(Time2=Time, Time=Time-7900)
dt3_3 <- dt3 %>% filter(between(Time,18000,30000)) %>% mutate(Time2=Time, Time=Time-18000)
...
write_csv(dt3_3, "/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/ea/data/b3_a_ot_ps_tgjl.csv")
...
make.model <- function(dt){
  model.base <- lmer(ps ~ (ot1+ot2+ot3) + (ot1+ot2+ot3| participant_phone), data = dt)
  model.0 <- lmer(ps ~ (ot1+ot2+ot3)+Target + (ot1+ot2+ot3| participant_phone), data = dt)
  model.1 <- lmer(ps ~ ot1*Target + (ot1+ot2+ot3| participant_phone), data = dt)
  model.2 <- lmer(ps ~ (ot1+ot2)*Target + (ot1+ot2+ot3| participant_phone), data = dt)
  model.3 <- lmer(ps ~ (ot1+ot2+ot3)*Target + (ot1+ot2+ot3| participant_phone), data = dt)
  
  m.comparsion <- anova(model.base, model.0, model.1, model.2, model.3)
  ...
  f <- list(m.comparsion, p.base, s.base, p.0, s.0, p.1, s.1, p.2, s.2, p.3, s.3)
}
...
model.a <- lmer(ps ~ (ot1+ot2+ot3)*Target + (ot1+ot2+ot3| participant_phone), data = dt3_a)
s.a <- summary(model.a)
p.a <- ggplot(dt3_a, aes(x=Time2, y=ps, color=Target)) +
                     stat_summary(fun.data=mean_se, geom="pointrange",alpha=0.75, size=0.3) +
                     stat_summary(aes(y=predict(model.a,dt3_a,re.form=NA)), fun.y=mean, geom="line") +
                     labs(title = "m.3")
