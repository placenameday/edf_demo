...
make.model2 <- function(dt, aoi){
  targetlevel <- c("LTA", "HTA")
  AOI_ne <- c("neutral", "threat",  "dysphoric", "positive")
  AOI_pos <- c("positive", "neutral", "threat",  "dysphoric")
  AOI_neg <- c("dysphoric","neutral", "threat", "positive")
  AOI_thr <- c("threat",  "dysphoric", "positive", "neutral")
  ...
  model.5 <- lmer(Elog ~ (ot1+ot2+ot3+ot4)*AOI*Target + (1| participant_phone), data = dt)
  ...
  f <- list(targetlevel, info, s.3, s.4, s.5)
}
