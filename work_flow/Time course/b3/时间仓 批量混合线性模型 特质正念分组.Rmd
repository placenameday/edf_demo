...
## 摘要
针对注视比例出时序图并进行mixed模型差异分析, 按照特质正念进行分组
...
write_csv(nk, "/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/ea/data/b3_timec_pwc_maas.csv")

nk2 <- nk  %>% select(Target, Time, b3_pos, b3_neg, b3_thr, b3_ne) %>%
  distinct() %>% pivot_longer(3:6, names_to = "AOI", values_to = "rank")
