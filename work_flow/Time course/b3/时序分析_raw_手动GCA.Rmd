...
make.model <- function(dt){
  ...
  model.0 <- lmer(Elog ~ (ot1+ot2+ot3+ot4)+AOI + (1| participant_phone), data = dt)
  model.1 <- lmer(Elog ~ ot1*AOI + (1| participant_phone), data = dt)
  model.2 <- lmer(Elog ~ (ot1+ot2)*AOI + (1| participant_phone), data = dt)
  model.3 <- lmer(Elog ~ (ot1+ot2+ot3)*AOI + (1| participant_phone), data = dt)
  model.4 <- lmer(Elog ~ (ot1+ot2+ot3+ot4)*AOI + (1| participant_phone), data = dt)
