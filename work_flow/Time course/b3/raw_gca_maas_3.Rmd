...
make.model2 <- function(dt, aoi){
  ...
  model.3 <- lmer(Elog ~ (ot1+ot2+ot3)*AOISum*Target + (1| participant_phone), data = dt)
  model.4 <- lmer(Elog ~ (ot1+ot2+ot3+ot4)*AOISum*Target + (1| participant_phone), data = dt)
...
tlist <- c("ne","pos","neg","thr")

f3_1 <- map(tlist, make.model2, dt=dt3_1)
f3_2 <- map(tlist, make.model2, dt=dt3_2)
f3_3 <- map(tlist, make.model2, dt=dt3_3)
