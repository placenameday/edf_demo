...
dttime <- dt3_1 %>% group_by(participant_phone) %>%
  summarise(tmin = min(time, na.rm = T), tmax = max(time, na.rm = T))
...
dttime2 <- full_join(dttime, msg) %>% filter(maas_g!="mid") %>% drop_na()

d3 <- dttime2 %>% mutate(duration = (tmax-tmin)/1000) %>% filter(duration>360)
mean(d3$duration)
...
# 保存时序数据
write_csv(response_time, "/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/ea/data/raw_time_b3.gz")
