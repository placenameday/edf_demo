...
mdt <-function(fpath){
  ...
  if (nrow(dt)==0) {
    NULL
  } else {dt <- dt %>% select(1:15, b3_pos, b3_neg, b3_thr, b3_ne)
    dt2 <- dt %>% mutate(trackloss=case_when((is.na(xp)|is.na(yp))~TRUE,
                                  !(is.na(xp)|is.na(yp))~FALSE))
  }
}
...
dirpath <- "/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/tidy/raw_clean_aoi"
opath <- "/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/tidy/b3_raw.csv.gz"
...
# 进行初步时序分析
```{r, message=FALSE, warning=FALSE}
dt3 <- fread("/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/tidy/b3_raw.csv.gz") %>%
...
# 保存时序数据
write_csv(response_time, "/Users/placenameday/R study/edf_demo/data/processed/eye_tacking/ea/data/raw_time.gz")
