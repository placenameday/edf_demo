...
dt3$Target <- factor(dt3$Target, levels=c("low", "high"))
...
nk2 <- nk  %>% select(Target, Time, b3_pos, b3_neg, b3_thr, b3_ne) %>%
  distinct() %>% pivot_longer(3:6, names_to = "AOI", values_to = "Advantages")
...
### 进行叠加可视化
```{r, message=FALSE, warning=FALSE}
dt_v <- full_join(dt2, nk2) %>%
  mutate(Advantages=factor(Advantages, levels=c(0,1,2,3)))

dt_rank <- dt_v %>% group_by(Target, Time, AOI) %>%
  mutate(Advantages=as.numeric(Advantages)) %>%
  summarise(Advantages=mean(Advantages, na.rm=T))

p3 <- ggplot(dt_rank, aes(x=Time, y=AOI, fill=Advantages)) +
  facet_grid(rows = vars(Target)) +
...
p2 <- ggplot(dt_v, aes(x=Time, y=Prop,color=AOI, fill=AOI, shape=Advantages)) +
  facet_grid(rows = vars(Target)) +
...
### 进行时序区间划分
```{r, message=FALSE, warning=FALSE}
timezone <- nk %>% group_by(Target, Time) %>%
  summarise(pairsn = n())
