...
ve_c_t <- function(dic){
  file_list <- paste(dic,"/",list.files(dic), sep = "")
  ilist <- future_map(file_list, vali_eye_t)
  bind_rows(ilist)
}
...
对raw文件全部清洗逐一输出
```{r, message=FALSE, warning=FALSE}
loss_clean <- function(file){
  dt <- fread(file)
  if (unique(as.numeric(dt$participant_phone)[[1]]) %in% pc$participant_phone) {
    return(NA)
  } else
    dt <- filter(dt, !is.na(participant_phone))
...
raw_clean_save <- function(rdic, sdic){
  ...
  map2(file_list, out_list, ~try(write_excel_csv(loss_clean(.x), .y)))
  "done"
}
